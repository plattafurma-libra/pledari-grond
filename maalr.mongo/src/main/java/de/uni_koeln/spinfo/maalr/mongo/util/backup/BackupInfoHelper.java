...
public class BackupInfoHelper extends AbstractBackupHelper {
	...
	public BackupInfos getBackupInfos() {
		...
		try {
			for (File file : backupFiles) {
				...
				list.add(new FileInfo().absolutePath(file.getAbsolutePath()).lastModified(getLastModified(file))
						.name(file.getName()).parent(file.getParentFile().getAbsolutePath())
						.creationDate(getCreationDate(file)).size(getSize(file)));
			}
		} catch (Exception e) {
	...
	private String getCreationDate(File file) {
		return new SimpleDateFormat("EEE, d MMM yyyy HH:mm:ss").format(new Date(file.lastModified()));
	}
