...
public class SuggestedEntryOptionsCell extends AbstractCell<LexEntryCellWrapper> {
	...
	interface EntryListUiRenderer extends UiRenderer {
		...
		void onBrowserEvent(SuggestedEntryOptionsCell cell, NativeEvent event, Element element, LexEntryCellWrapper wrapper);
	}
	...
	public SuggestedEntryOptionsCell(AllEntriesList list, ListFilter listFilter, final EditorConstants constants, final EditorMessages messages) {
		super("click");
		this.list = list;
	...
	void onReject(ClickEvent event, Element parent, final LexEntryCellWrapper wrapper) {
		...
		if(wrapper.getLexEntry().getCurrent() == null || !wrapper.getLexEntry().getCurrent().isApproved()) {
			...
			Command ok = new Command() {
				...
				@Override
				public void execute() {
					service.reject(wrapper.getLexEntry(), wrapper.getLemmaVersion(), callback);
				}
			};
			Dialog.confirm(constants.confirmDeletion(), constants.deleteEntryQuestion(), constants.confirmDeleteButton(), constants.cancelDeleteButton(), ok , null, true);
		}
